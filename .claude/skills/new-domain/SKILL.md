...
CLAUDE.md 절대규칙과 docs/conventions.md를 전제로 한다. 표준 구현체는 domain/store(설정형)이며,
목록·페이징 CRUD는 현재 브랜치에 product 실구현이 있을 때만 domain/product를 참고한다.

## 0단계. 작업 브랜치 확인

- 현재 브랜치와 작업 트리를 확인한다. 다른 작업의 미커밋 변경이 있으면 덮어쓰거나 섞지 않는다.
- `feature/<도메인명>` 브랜치가 아니면 구현 전에 브랜치를 만들거나, 이미 있는 변경을 어떻게 옮길지 사용자에게 확인한다.
- 기능 하나 = 브랜치 하나 = PR 하나 원칙을 구현 시작부터 적용한다.

## 1단계. 스펙 작성·확정 (필수 — approved가 아니면 훅이 운영 코드 생성을 차단한다)

1. `docs/specs/<도메인>.md`가 있는지 확인하고 frontmatter의 `status`를 본다.
   - `approved`: 내용을 읽고 2단계로 간다.
