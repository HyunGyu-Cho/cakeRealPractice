...
기능 하나를 만들려면 파일을 여러 개 만들어야 한다. 화면, 요청을 받는 곳, 업무 판단을 하는 곳,
DB에 SQL을 보내는 곳… **처음에는 "그래서 뭘 어디에 쓰라는 거지?"가 가장 막막하다.**

그래서 이 저장소는 **완성된 기능 하나를 본보기로 정해 뒀다.** `domain/store`(매장 정보 관리)다.
새 기능을 만들 때 판단이 서지 않으면 **store 코드를 그대로 따라 하면 된다.**

이 문서는 그 본보기가 **어떤 파일을 어떤 순서로 거쳐 동작하는지** 보여주는 지도다.

> **"수직 슬라이스"란** 화면부터 DB까지, 한 기능에 필요한 모든 계층을 한 벌로 갖춘 것을 말한다.
> 케이크를 위에서 아래로 한 조각 자른 모양을 떠올리면 된다 — 층이 전부 들어 있다.

> 규칙의 정본은 [`conventions.md`](conventions.md), **동작의 정본은 store 소스 코드**다.
> 이 문서는 흐름 지도이지 코드 사본이 아니다 — 세부는 항상 코드를 본다.
> 모르는 용어는 [용어 사전](conventions.md#용어-사전).
