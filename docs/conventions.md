...
| 부 | 내용 |
|---|---|
| [0부. 처음 보는 사람을 위해](#0부-처음-보는-사람을-위해) | 계층 구조 그림과 **용어 사전** — 모르는 단어가 나오면 여기 |
| [1부. 백엔드](#1부-백엔드) | 패키지 구조, DB 규약, 엔티티·DTO·매퍼·서비스·컨트롤러 |
| [2부. 프론트엔드 템플릿 규격](#2부-프론트엔드-템플릿-규격) | Thymeleaf 화면 구조, 공통 클래스, 디자인 토큰 |
| [3부. 상태값(status) 공통 규칙](#3부-상태값status-공통-규칙) | 전 도메인 status 표준과 전수 인벤토리 |

DB 스키마·마이그레이션은 [`database.md`](database.md), 확정된 업무 규칙은 [`business-rules.md`](business-rules.md)를 본다.
...
여섯 사람이 각자 다른 기능을 만들면, 같은 일을 서로 다른 방식으로 짜게 된다.
누구는 화면 코드에서 DB를 바로 부르고, 누구는 한글을 DB에 저장하고, 누구는 검증을 빠뜨린다.
나중에 합칠 때 그 차이가 전부 버그로 나타난다.

이 문서는 **"이런 일은 이렇게 한다"를 미리 하나로 정해 둔 것**이다.
판단이 서지 않으면 `domain/store` 코드를 그대로 따라 하면 된다.

## 코드가 어떻게 나뉘어 있나

기능 하나를 만들면 파일이 여러 개 생긴다. 각자 맡은 일이 다르기 때문이다.

```
브라우저에서 "매장 정보 저장" 버튼 클릭
   │
   ▼
Controller  ── 요청을 받고, 입력이 올바른지 확인하고, 결과 화면을 정한다
   │           (업무 판단은 하지 않는다)
   ▼
Service     ── 실제 업무 규칙을 실행한다. "재고가 모자라면 주문 거절" 같은 판단이 여기 있다
   │           (여러 작업을 한 묶음으로 처리 = 트랜잭션)
   ▼
Mapper      ── DB에 SQL을 보낸다. SQL은 XML 파일에 따로 적는다
   │
   ▼
MariaDB
```

**방향은 한쪽뿐이다.** Controller가 Mapper를 바로 부르거나, Service가 Controller를 부르는 일은 없다.
이 규칙 하나로 "이 로직이 어디 있지?"를 찾는 시간이 크게 줄어든다.

## 용어 사전

문서 곳곳에 나오는 말들이다. 모르는 단어가 보이면 여기로 돌아온다.

### 구조

| 용어 | 뜻 |
|---|---|
| **도메인(domain)** | 하나의 업무 영역. 이 프로젝트에는 회원·상품·장바구니·주문 등 13개가 있다 |
| **수직 슬라이스** | 화면부터 DB까지 한 기능에 필요한 전 계층을 한 벌로 만든 것. `domain/store`가 팀 표준 예시다 |
| **엔티티(Entity)** | DB 테이블 **한 행**을 담는 자바 객체. `Member`는 `members` 테이블 한 줄 |
| **DTO** | 계층 사이에 데이터를 나르는 그릇. 두 종류다 — **form**(화면→서버 입력), **view**(서버→화면 출력) |
| **MyBatis** | 자바와 SQL을 연결해 주는 도구. SQL을 우리가 직접 쓴다(자동 생성 아님) |
| **Mapper** | MyBatis에서 SQL을 부르는 창구. 인터페이스 + 짝이 되는 XML 파일 |

### 동작

| 용어 | 뜻 |
|---|---|
| **트랜잭션(transaction)** | 여러 DB 작업을 **전부 성공 아니면 전부 취소**로 묶는 것. 결제는 됐는데 재고가 안 줄어드는 상태를 막는다 |
| **롤백(rollback)** | 트랜잭션 중간에 실패해서 지금까지 한 걸 전부 되돌리는 것 |
| **PRG (Post-Redirect-Get)** | 폼 저장 후 결과 화면으로 **주소를 바꿔** 이동하는 방식. 안 그러면 새로고침할 때마다 같은 주문이 또 들어간다 |
| **검증(validation)** | 입력이 규칙에 맞는지 확인. 화면 입력은 form DTO에서, 업무 규칙은 Service에서 본다 |
| **멱등(idempotent)** | 몇 번 실행해도 결과가 같은 것. "읽음 처리"를 두 번 눌러도 한 번과 같아야 한다 |
| **팬아웃(fan-out)** | 하나의 사건으로 여러 명에게 각각 보내는 것. 주문이 들어오면 관리자 전원에게 알림 |

### 데이터

| 용어 | 뜻 |
|---|---|
| **스키마(schema)** | 테이블의 뼈대. 어떤 칸이 있고 어떤 타입인지 |
| **마이그레이션** | 스키마를 바꾸는 작업 지시서 파일. 자세히는 [`database.md`](database.md) |
| **시드(seed)** | 처음에 미리 넣어 두는 데이터. 카테고리 목록, 샘플 계정 등 |
| **enum** | 정해진 값만 골라 쓸 수 있는 목록. 주문 상태는 7개 중 하나뿐 |
| **상태값(status)** | 지금 어떤 단계인지. `PAID`(결제됨) → `PICKED_UP`(픽업완료) 처럼 바뀐다 |
| **전이(transition)** | 상태가 바뀌는 것. 아무 상태로나 못 가고 정해진 길만 갈 수 있다 |
| **파생값** | 저장하지 않고 **그때그때 계산**하는 값. "품절"은 재고가 0인지 보고 판단하지 별도로 저장하지 않는다 |
| **스냅샷** | 그 시점의 값을 복사해 두는 것. 주문 후 상품 가격이 바뀌어도 주문서 금액은 그대로여야 한다 |
| **UNIQUE 제약** | 중복을 DB가 직접 막는 것. 같은 주문에 결제가 두 번 들어가지 못하게 |
| **FK (외래 키)** | 다른 테이블의 행을 가리키는 연결. 주문은 어떤 회원의 것인지 가리킨다 |

### 작업 방식

| 용어 | 뜻 |
|---|---|
| **스펙(spec)** | 코드보다 **먼저** 쓰는 설계 합의서. `docs/specs/<도메인>.md` |
| **훅(hook)** | 특정 순간에 자동 실행되는 검사. 스펙 없이 코드를 쓰려 하면 막는다 |
| **스킬(skill)** | Claude에게 주는 작업 절차서. `/new-domain` 처럼 부른다 |
...
- **enum**: DB 컬럼은 `VARCHAR`로 둔다. MyBatis 기본 핸들러가 enum을 `name()` 문자열로 저장·조회하므로 별도 설정이 필요 없다. `INT`(ordinal) 저장은 금지한다. 상태 컬럼의 상세 규칙은 [3부](#3부-상태값status-공통-규칙)를 따른다.
...
- **상태 enum 컬럼**: 표준 방식은 [3부](#3부-상태값status-공통-규칙)에서 확정했다. `OrderStatus`·`PaymentStatus`가 모범 구현이며, 도메인별 미확정 값만 담당자가 채운다.

---

# 2부. 프론트엔드 템플릿 규격

개별 화면은 공통 CSS와 프래그먼트를 재사용하고, 화면 고유 마크업만 각 템플릿에 작성한다.
API·서비스·DB 연동 방식은 이 규격의 범위가 아니다(1부 참고).
