...
> 관리자 숨김·복구와 답글. 알림 발행과 후기 신고는 범위 밖이다.
...
- 확정한 규칙:
  ...
  10. **알림은 발행하지 않는다.** 확정된 `NotificationType` 12개와 team-plan 8장을 이번 범위에서 건드리지 않는다.
      후기 답글 알림이 필요해지면 notification 오너와 합의해 별도 사이클로 추가한다.
