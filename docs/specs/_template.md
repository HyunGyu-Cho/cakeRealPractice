---
domain: <도메인명>
status: draft
approved-at:
---

# <도메인명> 스펙

> **📄 스펙 문서가 뭔가요?** — 코드를 쓰기 **전에** "무엇을, 어떤 규칙으로 만들지" 먼저
> 못 박아 둔 설계 합의서다. 이 문서가 `status: approved`가 되기 전에는 훅이 구현 코드 작성을
> 막는다. 나중에 "왜 이렇게 만들었지?"의 답이 여기 있다.
>
> **이 도메인을 한 줄로** — <처음 보는 사람에게 설명하듯 쉬운 말 한 문장으로. 예: "회원이 살 케이크를 담아 두는 곳">
>
> 모르는 용어가 나오면 [용어 사전](../conventions.md#용어-사전)을 본다.

<!-- 위 안내 블록은 그대로 두고 "이 도메인을 한 줄로"만 채운다. 모든 스펙이 같은 형식을 갖는다. -->

> 이 파일을 `docs/specs/<도메인>.md`로 복사해 작성한다. 구현 착수 전에 내용을 확정해야 하며,
> `status: approved`가 아닌 도메인의 운영 코드 생성·수정은 훅이 차단한다. 확정 과정에서
> `docs/business-rules.md`(확정 업무 규칙)의
> 관련 미결 항목이 있으면 함께 채우고, 상태값을 새로 정하면 conventions.md 인벤토리도 갱신한다.

## 1. 개요와 유스케이스

- 이 도메인이 해결하는 문제 한 줄:
- 주요 유스케이스 (액터 → 행동 → 결과):
  1.
  2.

## 2. 상태값 (conventions.md 상태값 공통 규칙 준수)

| 컬럼 | 값(영문 enum 이름) | 시작 상태 | 최종 상태 | 전이 규칙 요약 |
|---|---|---|---|---|
| | | | | |

- status가 아닌 것 점검: 불리언인가? 파생값인가? type/category인가? 다른 도메인의 status인가?

## 3. DB

- 사용할 테이블 (V0 기준):
- 스키마 변경 필요 여부: 필요 시 `src/main/resources/db/migration/V<다음번호>__<설명>.sql` 신규 추가(커밋된 파일은 수정 금지)

## 4. 도메인 간 인터페이스

- 내가 제공할 공개 Service 메서드:
- 내가 사용할 다른 도메인의 공개 Service 메서드 (Mapper 직접 호출 금지):

## 5. 화면

- 대상 화면과 URL (목업 → 실구현 전환 시 URL·템플릿 유지):
- 목업 JS가 시연하는 임시 동작 중 규칙으로 확정할 것:

## 6. 비즈니스 규칙 확정

- `business-rules.md`에서 이 도메인과 관련된 규칙:
- 확정한 규칙 (확정 후 `business-rules.md` 갱신):

## 7. 완료 기준

- [ ] 정상 흐름·주요 실패 흐름 동작
- [ ] 입력 검증(form DTO)과 접근 권한 적용
- [ ] 상태 전이·트랜잭션 규칙 준수
- [ ] 전용 테스트 통과 (Service + Controller, store 패턴)
- [ ] 관련 SQL·문서 함께 수정
