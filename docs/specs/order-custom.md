...
- 확정한 규칙 (확정 후 team-plan 8장 표 갱신):
  ...
  9. **픽업 일시 검증.** order-payment.md의 픽업 규칙(매장 영업일·휴무일·영업시간·슬롯 간격 교집합,
     ...
     **견적의 `producible_date`**다. 요청서 제출 시점에는 상품 `preparation_days`를 하한으로 쓰고,
     견적 수락 시 `producible_date` 기준으로 재검증한다. 희망 픽업일이 제작 가능일보다 이르면
     관리자가 견적에서 `producible_date`를 제시하고 고객이 수락하면서 `pickup_at`을 다시 고른다.
...
- [ ] 정상 흐름·주요 실패 흐름 동작 (제출 → 견적 → 재견적 → 수락 → 결제 → 제작 시작, 반려·취소 포함)
- [ ] 입력 검증(form DTO)과 접근 권한 적용 (타인 요청·타인 링크 접근 차단)
- [ ] 상태 전이·트랜잭션 규칙 준수 (`OrderStatus` 무변경, 견적·링크 전이는 각 enum이 소유)
- [ ] 전용 테스트 통과 (Service + Controller, store 패턴). 만료·중복 결제·재견적 무효화 케이스 포함
- [ ] enum ↔ DDL CHECK 동기화 테스트 (notification 패턴)
- [ ] 관련 SQL·문서 함께 수정 (V13 + V0/V1 소급, README 현황표, TodoList, team-plan 8장)
