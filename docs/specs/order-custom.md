...
>
> **이 도메인을 한 줄로** — 가격이 미리 정해지지 않은 수제 케이크를 "요청 → 관리자 견적 → 결제" 순서로 진행한다.
>
...
- 확정한 규칙 (확정 후 team-plan 8장 표 갱신):
  ...
  9. **픽업 일시 검증.** order-payment.md의 픽업 규칙(매장 영업일·휴무일·영업시간·슬롯 간격 교집합,
     ...
     **견적의 `producible_date`**다. 요청서 제출 시점에는 상품 `preparation_days`를 하한으로 쓰고,
     ...
     - **픽업 예약 창은 유형별로 다르다.** 일반 주문은 14일(결제 시 재고가 묶이므로 짧게),
       주문제작은 **90일**이다(`CustomOrderService.PICKUP_WINDOW_DAYS`). 주문제작 상품은
       `stock_quantity`가 NULL이라 먼 미래 예약이 재고를 묶지 않고 실무 리드타임이 2주를 넘는다.
       `StoreService.getAvailablePickupSlots(date, preparationDays, windowDays)` 오버로드로
       호출측이 창을 지정한다. 관리자 견적의 `producible_date`가 90일을 넘으면 `CUSTOM_020`으로 막는다.
  13. **픽업 슬롯 정원은 1건이다.** 이미 픽업이 잡힌 시각은 슬롯 목록에서 빠지고, 그 시각을 직접
      찔러도 `STORE_008`로 거부된다. 취소·반려된 주문의 슬롯은 다시 열린다.
      - 예약 현황 조회는 store가 정의한 `PickupReservationPort`를 **order가 구현**해서 넘긴다.
        store가 `orders`를 직접 읽으면 order → store 위에 store → order가 얹혀 순환이 되기 때문이다.
  14. **픽업 예정 주문이 있는 날짜는 휴무일로 지정할 수 없다**(`STORE_009`). 관리자에게 몇 건인지
      함께 알려 주문을 먼저 정리하도록 한다. 이 규칙이 없으면 휴무일을 나중에 추가했을 때
      주문과 매장 일정이 조용히 어긋난다.
  15. **휴무일 픽업은 사유를 구분해 알린다**(`STORE_007`). 기존에는 모든 실패가
      "선택할 수 없는 픽업 일시입니다"로 뭉뚱그려져 고객이 이유를 알 수 없었다.
